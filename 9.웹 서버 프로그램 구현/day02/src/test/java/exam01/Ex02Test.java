...
public class Ex02Test {
    ...
    void test1() {
        ...
        then(mailer).should().send(email); // send 함수가  loginService.process 메서드에서 단 한번 호출되는지 테스트
    }
    ...
    void test2() {
        ...
        String userEamil = captor.getValue(); // send 메서드에서 매개변수로 사용한 값
        System.out.println(userEamil);

        assertEquals(email, userEamil);
