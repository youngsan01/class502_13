...
public class LoginServiceTest {
    ...
    void requiredFieldTest() {
        assertAll(
                // 이메일 검증
                () -> requiredFieldEachTest("eamil", "이메일"),
                //비밀번호 검증
                () -> {
                    setParamData("email", faker.internet().emailAddress());
    ...
    void requiredFieldEachTest() {
        // 아이디 필수 항목 검증 - null, " "
        ValidationException thrown = assertThrows(ValidationException.class, () -> {
            setParamData("email", faker.internet().emailAddress()); // 실패했을때 예외가 발생해서 통과
            loginService.process(request);
        });
    ...
    @Test
    @DisplayName("이메일로 회원이 조회되는지 검증, 검증 실패시 VaildatrionException 발생")
